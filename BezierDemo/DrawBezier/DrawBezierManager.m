...
#pragma mark - 增加获取曲线接口 tondy
- (UIBezierPath*)getPathWithPoints:(NSArray *)touchPoints
{
    for (float i = 0; i < 1.01; i += 0.01) {
        [self getAllPointsWithOriginPoints:touchPoints progress:i];
    }
    NSArray *points = [NSArray arrayWithArray:_bezierPathPoints];
    //创建路径
    CGMutablePathRef path = CGPathCreateMutable();
    
    for (int i = 1; i < points.count; i ++) {
        NSValue *startPointValue = [points objectAtIndex:i-1];
        NSValue *endPointValue = [points objectAtIndex:i];
